# src/mycli/commands/who_new.py
# Mismo comportamiento que who_old, pero usando who_new_path
from pathlib import Path
from mycli.utils import list_dirs, list_media_files, prompt_choice, open_with_default

def register_parser(subparsers):
    subparsers.add_parser('who-new', help='Navegar Doctor Who Moderno')

def run(args, cfg):
    base = cfg.get('who_new_path')
    player = cfg.get('player_cmd')
    if not base:
        print("Error: who_new_path no configurado en la config.")
        return
    base_path = Path(base)
